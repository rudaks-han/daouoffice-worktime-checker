...
export default class WorkHourChecker {
	...
	check = async params => {
		Logger.debug("######### WorkHourChecker check start");
		// 주말인지 여부 체크
	...
	isWeekend = params => {
		...
		if (date.getDay() == 0 || date.getDay() == 6) { // 토, 일 제외
			Logger.trace(`${logPrefix} 오늘은 주말입니다. ${currDate}`);
			return true;
		} else {
			Logger.trace(`${logPrefix} 오늘은 평일입니다. ${currDate}`);
			return false;
		}
	}
	...
	isHoliday = params => {
		...
		if (holidayList[currDate]) {
			Logger.trace(`${logPrefix} 오늘은 공휴일입니다. ${currDate}`);
			return true;
		} else {
			Logger.trace(`${logPrefix} 오늘은 평일입니다. ${currDate}`);
			return false;
