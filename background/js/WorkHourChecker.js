...
class WorkHourChecker {
	...
	getUserConfig()
	{
		...
		Share.promiseStorageSync('clock-in-hour')
			.then(() => Share.promiseStorageSync('clock-in-minute'))
			.then(() => {
			...
			.then(() => Share.promiseStorageSync('clock-out-hour'))
			.then(() => Share.promiseStorageSync('clock-out-minute'))
			.then(() => {
				return new Promise(function(resolve, reject) {
					setTimeout(function() {
			...
			.then(() => Share.promiseStorageSync('clock-in-check-type', 'clockInCheckType'))
			.then(() => Share.promiseStorageSync('clock-in-before-minute', 'minuteBeforeClockIn'))
			.then(() => Share.promiseStorageSync('clock-in-random-from-minute', 'clockInRandomFromMinute'))
			.then(() => Share.promiseStorageSync('clock-in-random-to-minute', 'clockInRandomToMinute'))

			.then(() => Share.promiseStorageSync('clock-out-check-type', 'clockOutCheckType'))
			.then(() => Share.promiseStorageSync('clock-out-after-minute', 'minuteAfterClockOut'))
			.then(() => Share.promiseStorageSync('clock-out-random-from-minute', 'clockOutRandomFromMinute'))
			.then(() => Share.promiseStorageSync('clock-out-random-to-minute', 'clockOutRandomToMinute'))
	...
	requestCalendar()
	{
		...
		let url = `${this.BASE_URL}/api/calendar/user/me/event/daily?year=${Share.getCurrYear()}&month=${Share.getCurrMonth()}`;
	...
	isMarkedClockOutAlready()
	{
		...
		let storageClockOutDate = Share.getLocalStorage('CLOCK_OUT_DATE');

		if (storageClockOutDate == Share.getCurrDate()) {
...
export default WorkHourChecker;
